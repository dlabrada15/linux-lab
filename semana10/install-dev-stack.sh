...
sudo apt purge-y "${PAQUETES_INSTALADOS[@]}" 2>/
dev/null || true
sudo apt autoremove-y 2>/dev/null || true
log OK "Rollback completado. Paquetes revertidos: $
{PAQUETES_INSTALADOS[*]}"
fi                                                                                                                                                                       }
trap rollback EXIT
...
local version
version=$(dpkg-l "$paquete" | grep "^ii" | awk ’{
print $3}’)
log OK "$descripcion ya instalado (v$version)-
omitido"
PAQUETES_OMITIDOS+=("$paquete")
else
log INFO "Instalando $descripcion ($paquete)..."
if sudo $PKG_INSTALL "$paquete" >> "$LOG_FILE"
2>&1; then
...
instalar_paquete "git" "Git (control de
versiones)"
instalar_paquete "curl" "curl (transferencia HTTP)"
instalar_paquete "wget" "wget (descarga de
archivos)"
instalar_paquete "vim" "Vim (editor de texto)
"
instalar_paquete "tree" "tree (visualizar
directorios)"
instalar_paquete "htop" "htop (monitor de
procesos)"
...
instalar_paquete "python3-pip" "pip3 (gestor paquetes
Python)"
...
instalar_paquete "tmux" "tmux (terminal
multiplexer)"
instalar_paquete "shellcheck" "ShellCheck (linter
Bash)"
}
...
log OK "Git ya configurado: $(git config--global
user.name)"
else
log WARN "Git no tiene nombre de usuario
configurado."                                                                                                                                                         log INFO "Ejecuta: git config--global user.name ’
Tu Nombre’"
log INFO "Ejecuta: git config--global user.email ’
tu@email.com’"
fi
}
...
log WARN "Instalacion completada con $ERRORES
errores"
