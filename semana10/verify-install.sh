#!/bin/bash
# verify-install.sh-Verifica que el dev stack este
instalado
...
echo-e " ${RED}[FAIL]${NC} $nombre ($pkg): no
instalado"
FAIL=$((FAIL + 1))
fi
}
...
echo-e " ${YELLOW}Estado: $FAIL herramientas
faltantes${NC}"
...
echo "Estado: $([ $FAIL-eq 0 ] && echo ’COMPLETO’ ||
echo ’INCOMPLETO’)"
